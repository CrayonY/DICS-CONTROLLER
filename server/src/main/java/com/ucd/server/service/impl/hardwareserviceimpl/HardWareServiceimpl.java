            ...
            if (obj != null) {
                ...
                if (HardWareTypeEnum.NIC.getValue().equals(type)) {
                    ...
                    list.stream().forEach((String string) -> {
                        ...
                        hardwareNicVOList3 = gson.fromJson(ListVOString, new TypeToken<List<HardwareNicVO>>() {
                        }.getType());
                ...
                if (HardWareTypeEnum.THREAD.getValue().equals(type)) {
                    ...
                    list.stream().forEach((String string) -> {
                        ...
                        Gson gson = new Gson();
                        // 将LinkedHashMap类型转换成可以识别的实体类
                        hardwareThreadVOList3 = gson.fromJson(ListVOString, new TypeToken<List<HardwareThreadVO>>() {
                        }.getType());
                        // 对数据进行筛选
